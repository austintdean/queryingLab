...
def problem_five(request):
    ...
    student_new = Student.objects.create(first_name="Austin",last_name ="Dean",year=2020,gpa=3.0)
    print(f"Full Name: {student_new.first_name} {student_new.last_name} Year:{student_new.year} GPA: {student_new.gpa}")
...
def problem_six(request):
    student_id = 11
    Student.objects.filter(id=11).update(gpa=4)
    student_eleven = Student.objects.filter(pk=11).get()
    print(f'Id: {student_id} Full Name:{student_eleven.first_name} {student_eleven.last_name} GPA: {student_eleven.gpa}')
